...
class PlayState extends MusicBeatState
{
	...
	public var oppdrain:Bool = false;
	...
	override public function create()
	{
		...
		oppdrain = ClientPrefs.getGameplaySetting('oppdrain', false);
		...
		switch (curStage)
		{
			...
			case 'school': //Week 6 - Senpai, Roses
				...
				var bgSky:BGSprite = new BGSprite('weeb/weebSky', 0, 0 + senpaiYOffset, 0.1, 0.1);
				add(bgSky);
				bgSky.antialiasing = false;

				var repositionShit = -200;

				var bgSchool:BGSprite = new BGSprite('weeb/weebSchool', repositionShit, 0 + senpaiYOffset, 0.6, 0.90);
				add(bgSchool);
				bgSchool.antialiasing = false;

				var bgStreet:BGSprite = new BGSprite('weeb/weebStreet', repositionShit, 0 + senpaiYOffset, 0.95, 0.95);
				add(bgStreet);
				bgStreet.antialiasing = false;

				var widShit = Std.int(bgSky.width * 6);
				if(!ClientPrefs.lowQuality) {
					var fgTrees:BGSprite = new BGSprite('weeb/weebTreesBack', repositionShit + 170, 130 + senpaiYOffset, 0.9, 0.9);
				...
				var bgTrees:FlxSprite = new FlxSprite(repositionShit - 380, -800 + senpaiYOffset);
				...
				if(!ClientPrefs.lowQuality) {
					var treeLeaves:BGSprite = new BGSprite('weeb/petals', repositionShit, -40 + senpaiYOffset, 0.85, 0.85, ['PETALS ALL'], true);
				...
				if(!ClientPrefs.lowQuality) {
					bgGirls = new BackgroundGirls(-100, 190 + senpaiYOffset);
			...
			case 'schoolEvil': //Week 6 - Thorns
				...
				var posY = 200;
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('weeb/animatedEvilSchool', posX, posY + senpaiYOffset, 0.8, 0.9, ['background 2'], true);
					bg.scale.set(6, 6);
					bg.antialiasing = false;
					add(bg);

					bgGhouls = new BGSprite('weeb/bgGhouls', -100, 190 + senpaiYOffset, 0.9, 0.9, ['BG freaks glitch instance'], false);
					...
					var bg:BGSprite = new BGSprite('weeb/animatedEvilSchool_low', posX, posY + senpaiYOffset, 0.8, 0.9);
					bg.scale.set(6, 6);
					bg.antialiasing = false;
					add(bg);
				}
			...
			case 'clear': //no stage at all

				var bg:FlxSprite = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.fromRGB(0, 0, 0));
					if (defaultCamZoom < 1)
					{
		...
		if(sys.FileSystem.exists('assets/shared/images/healthBarSkins/' + ClientPrefs.healthBarSkin + '.png')
		|| sys.FileSystem.exists('mods/' + Paths.currentModDirectory + '/images/healthBarSkins/' + ClientPrefs.healthBarSkin + '.png')
		|| sys.FileSystem.exists('mods/images/healthBarSkins/' + ClientPrefs.healthBarSkin + '.png')) {
			healthBarBG = new AttachedSprite('healthBarSkins/' + ClientPrefs.healthBarSkin);
		} else {
			healthBarBG = new AttachedSprite('healthBarSkins/Default');
		}
		...
		if(songName == "Reaction") {
			botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "FUCK YOU " + username, 32);
		} else {
	...
	function cacheCountdown()
	{
		...
		introAssets.set('default', ['Countdowns/' + ClientPrefs.uiCount + '/ready', 'Countdowns/' + ClientPrefs.uiCount + '/set', 'Countdowns/' + ClientPrefs.uiCount + '/go']);
		...
		Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/intro3' + introSoundsSuffix);
		Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/intro2' + introSoundsSuffix);
		Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/intro1' + introSoundsSuffix);
		Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/introGo' + introSoundsSuffix);
	}
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				{
					case 0:
						FlxG.sound.play(Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/intro3' + introSoundsSuffix), 0.6);
					case 1:
						...
						FlxG.sound.play(Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/intro2' + introSoundsSuffix), 0.6);
					case 2:
						...
						FlxG.sound.play(Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/intro1' + introSoundsSuffix), 0.6);
					case 3:
						...
						FlxG.sound.play(Paths.sound('Countdowns/' + ClientPrefs.uiCount + '/introGo' + introSoundsSuffix), 0.6);
					case 4:
				}
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				if (achievementName.contains(WeekData.getWeekFileName()) && achievementName.endsWith('gold')) // any golden achievements, name should be "weekFileName_gold", e.g: "weekd_gold";
				{
					if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'EXTRA POWER'
						&& storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						unlock = true;
				}
				switch(achievementName)
				{
					...
					case 'toastie':
						if(/*ClientPrefs.framerate <= 60 &&*/ !ClientPrefs.shaders && ClientPrefs.lowQuality && !ClientPrefs.globalAntialiasing && !ClientPrefs.ratings) {
							unlock = true;
						}
					case 'debugger':
