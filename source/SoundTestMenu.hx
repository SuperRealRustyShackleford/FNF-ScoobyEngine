...
class SoundTestMenu extends MusicBeatState
{
    ...
    override function create()
        {
			...
			new FlxTimer().start(0.1, function(tmr:FlxTimer)
				{
					FlxG.sound.playMusic(Paths.music('soundtest'));
				});
	...
	function flashyWashy(a:Bool)
	{
		if (a == true)
		{
			FlxG.sound.play(Paths.sound('sonicexe/confirmMenu'));
	...
	function doTheThing(first:Int, second:Int) 
	{
		...
		{
			PlayState.SONG = Song.loadFromJson('reaction-hard', 'reaction');
		...
		{
			PlayState.SONG = Song.loadFromJson('money', 'money');
