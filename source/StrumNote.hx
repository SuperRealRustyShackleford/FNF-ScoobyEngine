...
class StrumNote extends FlxSprite
{
	...
	public function new(x:Float, y:Float, leData:Int, player:Int) {
		...
		var skin:String = 'noteSkins/Default/default';
	...
	public function reloadNote()
	{
		...
		if(PlayState.isPixelStage)
			{
				frames = Paths.getSparrowAtlas('pixelUI/' + texture);
				antialiasing = false;
				setGraphicSize(Std.int(width * Note.scales[PlayState.mania] *= PlayState.daPixelZoom + 3));
			}
		else
			{
		...
		var animation_Base_Name = animationArray[Std.int(Math.abs(0))].toLowerCase();
