...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		DiscordClient.changePresence("Scooby Chart Editor", StringTools.replace(_song.song, '-', ' ') + " (" + (_song.mania + 1) + " key" + s_termination + ")");
		#end
	...
	function addNoteUI():Void
	{
		...
		for (i in 0...directories.length) {
			...
			if(FileSystem.exists(directory)) {
				for (file in FileSystem.readDirectory(directory)) {
					var path = haxe.io.Path.join([directory, file]);
					if (!FileSystem.isDirectory(path) && file.endsWith('.lua') || !FileSystem.isDirectory(path) && file.endsWith('.txt')) {
